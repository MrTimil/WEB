...
echo '<form method="POST" action="inscription_06.php">',
		'<table border="1" cellpadding="4" cellspacing="0">',
            '<tr>',
                '<td>Indiquez votre nom</td>',
                '<td><input type="text" name="txtNom" value="', 
                htmlentities($_POST['txtNom'], ENT_QUOTES, 'UTF-8'),  '" size="40"></td>',
            '</tr>',			
            '<tr>',
            '<td>Indiquez une adresse mail valide</td>',
                '<td><input type="text" name="txtMail" value="',  
                 htmlentities($_POST['txtMail'], ENT_QUOTES, 'UTF-8'), '" size="40"></td>',
            '</tr>',				
            '<tr>',
            ...
            '<tr>',
                ...
                for ($i = 1; $i <= 12 ; $i++){
                    if ($i == $_POST['selDate_m']){
                        echo  '<option value="', $i, '" selected>', fd_get_mois($i), '</option>';
                    }
                    else{
                        echo  '<option value="', $i, '">', fd_get_mois($i), '</option>';
                    }
                }    
                echo '</select> ',						
                    '<select name="selDate_a">';
                for ($i = 2017; $i >= 1987 ; $i--){
                    if ($i == $_POST['selDate_a']){
                        echo  '<option value="', $i, '" selected>', $i, '</option>';
                    }
                    else{
                        echo  '<option value="', $i, '">', $i, '</option>';
...
function fdl_add_utilisateur() {
	...
	header ('location: ../tp4/liste_users_02.php');
